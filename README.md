...
* Поддержаны функции из одного предложения, без блока `{ }`. Пока в таковых
разрешено лишь выражение-результат, либо отсутствие оного (аналог явного
определения идентификаторов посредством `$ENUM` Refal-05).
